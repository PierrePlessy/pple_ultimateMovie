var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@angular/core';
import { HTTP } from '@ionic-native/http/ngx';
import { HttpClient } from '@angular/common/http';
var OmdbService = /** @class */ (function () {
    function OmdbService(http, httpCLient) {
        this.http = http;
        this.httpCLient = httpCLient;
        this.baseURL = "http://www.omdbapi.com?";
        this.apiKey = "apiKey=75522b56";
    }
    OmdbService.prototype.search = function (term, type) {
        return this.httpCLient.get(this.baseURL + this.apiKey + "&type=" + type + "&s=" + term);
    };
    OmdbService.prototype.findById = function (id) {
        return this.httpCLient.get(this.baseURL + this.apiKey + "&i=" + id);
    };
    OmdbService.prototype.getOneSeason = function (id, n) {
        return this.httpCLient.get(this.baseURL + this.apiKey + "&i=" + id + "&Season=" + n);
    };
    OmdbService = __decorate([
        Injectable({
            providedIn: 'root'
        }),
        __metadata("design:paramtypes", [HTTP, HttpClient])
    ], OmdbService);
    return OmdbService;
}());
export { OmdbService };
//# sourceMappingURL=omdb.service.js.map